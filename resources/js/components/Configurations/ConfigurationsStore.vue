<template>
  <div id="createConfiguration" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <validation-observer v-slot="{ invalid }" ref="formconfigurationcreate">
        <form method="POST" class="modal-content" autocomplete="off" @submit.prevent="createConfigurationSubmit(configuration)">
          ...
          <div class="modal-footer">
            <button type="reset" class="btn btn-secondary btn-icon-split" data-dismiss="modal" @click.prevent="createConfigurationReset()">
...
export default {
  components: {
    modalConfigurationForm: modalConfigurationForm
  },
  props: {
    configuration: {
      type: Object,
  ...
  methods: {
    createConfigurationSubmit: function (configuration) {
      var data = {
      ...
      this.$refs.formconfigurationcreate.validate().then(success => {
        if (!success) {
          return
        }
        ...
        this.$http.post('/api/v1/configurations/', data).then((response) => {
          if (response.status === 201) {
            ...
            this.createConfigurationReset()

            this.$toad.toad('La configuracion se agrego correctamente')
          }
        })
      })
    },
    createConfigurationReset: function () {
      ...
      this.$refs.formconfigurationcreate.reset()
    }
  }
}
