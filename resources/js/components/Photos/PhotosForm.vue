      ...
      <template v-slot:before-outer>
        <div class="row">
          <div class="col-md-12 text-center">
            <img class="img-thumbnail rounded" :src="`${photo.url}`" :alt="`${photo.name}`" style="width: 320px;">
          </div>
            ...
            <div class="form-group">
              ...
              <validation-provider v-slot="{ errors, classes }" name="Nombre" rules="min:3|max:100|required">
                <div class="control" :class="classes">
                  <input v-model="photo.name" type="text" class="form-control lower--mdf" placeholder="Ingresa el nombre" maxlength="101">
                  <span>
                    {{ errors[0] }}
                  </span>
                </div>
          ...
          <div class="col-md-12">
            <div class="form-group">
              ...
              <validation-provider v-slot="{ errors, classes }" name="Descripcion" rules="min:4|max:250">
                <div class="control" :class="classes">
                  <textarea v-model="photo.description" class="form-control lower--mdf" rows="5" placeholder="Ingresa la descripcion de la caracteristica" maxlength="251" />
                  <span>
                    {{ errors[0] }}
                  </span>
                </div>
...
export default {
  ...
  methods: {
    removeFileData: function (fileData) {
      this.photo.url = this.photo.temp
