...
export default {
  ...
  methods: {
    edit: function (id) {
      this.$http.get('/photosgalleries/' + id).then((response) => {
        this.$emit('dataEdit', {
          id: response.data.id,
    ...
    deleted: function (id) {
      this.$swal.fire({
        ...
        width: '21rem',
        preConfirm: () => {
          this.$http.delete('/photosgalleries/' + id).then((response) => {
            if (response.status === 204) {
              this.$http.get('/photosgalleries/all/' + this.galleryid).then((response) => {
    ...
    editStatus: function (id, attr) {
      this.$swal.fire({
        ...
        width: '21rem',
        preConfirm: () => {
          this.$http.put('/photosgalleries/' + id, {
