...
export default {
  ...
  methods: {
    edit: function (id) {
      this.$http.get('/subcategories/' + id).then((response) => {
        this.$emit('dataEdit', {
          id: response.data.id,
    ...
    deleted: function (id) {
      this.$swal.fire({
        ...
        width: '21rem',
        preConfirm: () => {
          this.$http.delete('/subcategories/' + id).then((response) => {
            if (response.status === 204) {
              this.$http.get('/subcategories').then((response) => {
    ...
    editStatus: function (id, attr, categoryId) {
      this.$swal.fire({
        ...
        width: '21rem',
        preConfirm: () => {
          this.$http.put('/subcategories/' + id, {
